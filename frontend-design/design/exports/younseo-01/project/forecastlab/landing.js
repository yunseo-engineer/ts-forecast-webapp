...
function renderModelSparks() {
  ...
  function buildSeries(kind) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (kind === 'arima') {
        ...
        // Strong seasonality + trend changepoint at midpoint
        const cp = t < 0.5 ? -t * 30 : -15 + (t - 0.5) * 80;
        y = H * 0.55 + cp + Math.sin(t * Math.PI * 6) * 28;
      } else if (kind === 'ets') {
        // Smooth exponential — gentle damped curve
        y = H * 0.7 - (1 - Math.exp(-t * 3)) * 70 + Math.sin(t * Math.PI * 3) * 8;
      } else {
        // LSTM: complex nonlinear — irregular pattern
        y = H * 0.5
          + Math.sin(t * Math.PI * 3.2) * 18
          + Math.cos(t * Math.PI * 7) * 10
          + Math.sin(t * Math.PI * 13) * 6
          - t * 24;
      }
  ...
  document.querySelectorAll('.model-spark').forEach(host => {
    ...
    const color = kind === 'arima' ? c1 : kind === 'prophet' ? c2 : kind === 'ets' ? c3 : c1;
    const colorB = kind === 'arima' ? c2 : kind === 'prophet' ? c3 : kind === 'ets' ? c1 : c3;
    const id = 'sp-' + kind;
    host.innerHTML = `
